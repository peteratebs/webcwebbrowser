 ...
 |     (NOTE: if building RTIP, use rtpnet.c in rtpcore)
 |
 | EBS - RT-Platform
 |
 |  $Author: vmalaiya $
...
int rtp_net_gethostbyname (unsigned char *ipAddr, int *type, char *name)
{
	...
	if (getnameinfo( (LPSOCKADDR)res->ai_addr, res->ai_addrlen, addrName, sizeof(addrName), NULL, 0, NI_NUMERICHOST) != 0)
    {
            strcpy(addrName, "");
	}
...
int  rtp_net_setmcastttl(RTP_HANDLE sockHandle, int ttl)
{
    ...
    {
    	result = setsockopt( (SOCKET)sockHandle, IPPROTO_IP, IP_MULTICAST_TTL, (char *) &ttl, sizeof (int));
	}
	else if (localAddr.ss_family == AF_INET6)
	{
		result = setsockopt( (SOCKET)sockHandle, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, (char *) &ttl, sizeof (int));
	}
	else
	{
		return (-1);
...
int rtp_net_get_last_error(void)
{
