...
#if (USE_CLISRV)
#define SPAWN_WEB_SERVER 0  /* DON'T CHANGE */
#else
#define SPAWN_WEB_SERVER 1
#endif

#define MEMORY_POOL_SIZE       6*128*1024
static unsigned char    MemoryPool[MEMORY_POOL_SIZE];

/* in virttbl.c in application */
extern struct virtual_file_entry RTP_FAR *virtual_file_table;
...
int rtp_net_init (void)
{
	...
	{
	int iface0;
	RTP_UINT8 mask[] = {0xff, 0x0, 0x0, 0};  /* PETER - use local MASK */

        if (xn_bind_loop(PARAM_MINOR_0))
        {
            rtp_term_puts("xn_bind_loop failed");
        }

        iface0 = xn_interface_open (LOOP_DEVICE, PARAM_MINOR_0);
        if (iface0 < 0)
        {
            ; //RTP_DEBUG_ERROR("xn_interface_open failed", NOVAR, 0, 0);
        }

        if (xn_set_ip (iface0, my_ip, mask) != 0)
        {
            ; // RTP_DEBUG_ERROR("xn_set_ip failed", NOVAR, 0, 0);
        }

	}

    if (xn_bind_winpcap(0))
        return(-1);

    iface_no =
        xn_interface_open_config(WINPCAP_DEVICE,
                                 0,
                                 (IOADDRESS)ed_io_add,
                                 ed_irq,
                                 ed_mem_add);
    ...
    /* add gateway to routing table as default gateway             */
    if (xn_rt_add(RT_DEFAULT,       /* default */
                  RT_DEFAULT_MASK, 
                  ip_gw_address,
                  RT_USEIFACEMETRIC,
                  iface_no, 
                  RT_INF))
    {
        rtp_term_puts("xn_rt_add Failed");
        return(-1);
    }
	if (xn_add_dns_server(dns_ip_address))
	{
        rtp_term_puts("xn_add_dns_server failed");
        return(-1);
	}
...
#if (!USE_CLISRV)
    populate_ram_disk((KS_CONSTANT struct virtual_file_entry RTP_FAR *)&virtual_file_table);
#endif
#if (SPAWN_WEB_SERVER)
#if (CFG_SPAWN_WEB)
    start_web_server();
...
long rtp_net_send_ssl (RTP_SOCKET sockHandle, const unsigned char *buffer, long size, int flags)
{
    ...
    { 
        ...
        if (lastError == EINPROGRESS || lastError == EWOULDBLOCK) ret = -2;
    }
}
#endif
...
{    
    ...
    /* SO_RCV_TIMEO, SO_SEND_TIMEO take timeval struct */
    struct      timeval timeout;
...
{
#if (!POLLOS)
    while (web_spawned)     /* set to RTP_FALSE if server killed */
#endif
    {
#if (INCLUDE_WEB_SOAP)
        if (web_soap_server)
        {
            if (http_server_daemon_info(glb_ip_nulladdr, 5010) == -1)
            {
                RTP_DEBUG_ERROR("HTTP server failed!", EBS_NOVAR, 0, 0);
                ks_sleep(ks_ticks_p_sec());
            }
#if (!POLLOS)
            else
            {
                RTP_DEBUG_ERROR("HTTP server finished", EBS_NOVAR, 0, 0);
            }
#endif
        }
        else
...
RTP_BOOL web_spawned = RTP_FALSE;
...
char encoded_string3[BASE64_MAX_ENCODED_SIZE(USRPWDSIZE)];

struct web_auth_entry auth_info[] =
{
    /*                         user name,   space to                         */
    /* file        realm       password     encode usrpwd1    len   file     */
    /* ----        -----       ----------   ---------------   ---   ----     */
    {"survey.htm", "group2",   usrpwd1,     encoded_string1,  0,    0},
    {"index.html", "group1",   usrpwd2,     encoded_string2,  0,    0},
//    {"log_req.fn", "group2",   usrpwd1,     encoded_string3,  0,    "err.htm"}
//    {"log_req.fn", "group2",   usrpwd1,     encoded_string3,  0,    "errx.htm"}
    {"log_req.fn", "group2",   usrpwd1,     encoded_string3,  0,    0}
};
#endif
...
RTP_UINT8 ip_addr[4] = {192, 168, 0, 1};
RTP_UINT8 ip_mask[4] = {0xff, 0xff, 0xff, 0xff};
    ...
    {
        RTP_DEBUG_ERROR("http_set_browser_list failed", EBS_NOVAR, 0, 0);
    }
#endif
    ...
    if (!web_spawned)
    {
        web_spawned = RTP_TRUE;

#if (RTPLATFORM)
    if (rtp_thread_spawn (
                &new_thread,
                (RTP_ENTRY_POINT_FN) web_server_main,
                (const char *) "RTIP_WEB",
                STACKSIZE_NORMAL_INDEX,
                TASKPRIO_NORMAL_INDEX,
                (void *) 0
            ) < 0)
#else
        /* spawn an web server task           */
        if (!os_spawn_task(TASKCLASS_WEB_DAEMON,web_server_main,"RTIP_WEB",
                           0,0,0,0))
#endif
        {
            rtp_term_puts("spawn of WEB task failed");
            return;
        }
    }
}
#endif  /*  SPAWN_WEB_SERVER */
