/*
    ...
    *   Automatic  pool compaction, growth, and shrinkage by
    ...
    void bstats(bufsize *maxalloc, bufsize *curalloc, bufsize *totfree,
            bufsize *maxfree, long *nget, long *nrel);

    The maximum total amount of space that was ever allocated is
    stored in <maxalloc>. The amount  of  space  currently  allocated
    is  stored  into  the variable  pointed  to by <curalloc>.  The
    total free space (sum of all free blocks in the pool) is stored
    into the  variable  pointed to  by  <totfree>, and the size of the
    largest single block in the free space  pool  is  stored  into
    the  variable  pointed  to  by <maxfree>.   The  variables
    pointed  to  by <nget> and <nrel> are filled, respectively, with
    the  number  of  successful  (non-0 return) bget() calls and the
...
static bufsize maxalloc = 0;          /* Maximum Total space allocated */
...
static void* (*acqfcn) _((bufsize size)) = 0;
static void (*relfcn) _((void* buf)) = 0;
...
void* bget(bufsize requested_size)
{
   ...
   if(!(size > 0))
   {
   		RTIP_DEBUG_ERROR("bget size <= 0", EBS_NOVAR, 0, 0);
   		return(0);
   }
...
/*  while (b != &freelist) {   */
    while (b != freeptr) {
        ...
        if ((b->bh.bsize - size) > (SizeQ + (sizeof(struct bhead)))) {
            ...
            ba = BH(((RTP_PFINT8) b) + (b->bh.bsize - size));
            bn = BH(((RTP_PFINT8) ba) + size);
            
            if (! (bn->prevfree == b->bh.bsize))
            {
              RTIP_DEBUG_ERROR("bget: prevfree != bh.size ",EBS_NOVAR,0,0);
              return(0);
            }
...
#ifdef BufStats
            totalloc += size;
            if (totalloc>maxalloc) maxalloc = totalloc;
            numget++;         /* Increment number of bget() calls */
#endif
            buf = (void*) ((((RTP_PFINT8) ba) + sizeof(struct bhead)));
#if (DEBUG_BGET)
            RTIP_DEBUG_ERROR("bget: addr, len", EBS_DINT2, buf, requested_size);
#endif
            return buf;
        } else {
            struct bhead *ba;

            ba = BH(((RTP_PFINT8) b) + b->bh.bsize);
            
            if(!(ba->prevfree == b->bh.bsize))
            {
            	RTIP_DEBUG_ERROR("bget: ba->prevfree == b->bh.bsize",EBS_NOVAR,0,0);
            	return(0);
            }
            ...
            {
            	RTIP_DEBUG_ERROR("bget flink != b",EBS_NOVAR,0,0);
            	return(0);
           	}
            
            if(!(b->ql.flink->ql.blink == b))
            {
             	RTIP_DEBUG_ERROR("bget blink != b",EBS_NOVAR,0,0);
             	return(0);
            }
...
#if (DEBUG_BGET)
    RTIP_DEBUG_ERROR("bget: addr, len", EBS_DINT2, 0, requested_size);
#endif
    return 0;
}
...
void* bgetz(bufsize size)
{
    RTP_PFINT8 buf = (RTP_PFINT8) bget(size);
    ...
    RTIP_DEBUG_ASSERT(rsize >= size, "bgetz:", EBS_NOVAR, 0, 0);
...
void* bgetr(void* buf, bufsize size)
{
    ...
    b = BH(((RTP_PFINT8) buf) - sizeof(struct bhead));
    osize = -b->bsize;
#ifdef BECtl
    if (osize == 0) {
    ...
    bd = BDH(((RTP_PFINT8) buf) - sizeof(struct bdhead));
    osize = bd->tsize - sizeof(struct bdhead);
    } else
#endif
    osize -= sizeof(struct bhead);
    RTIP_DEBUG_ASSERT(osize > 0, "bgetr: ", EBS_NOVAR, 0, 0);
    rtp_memcpy((RTP_PFINT8) nbuf, (RTP_PFINT8) buf, /* Copy the data */
...
void brel(void* buf)
{
...
#if (DEBUG_BGET)
    RTIP_DEBUG_ERROR("brel: addr", EBS_DINT1, buf, 0);
#endif
    b = BFH(((RTP_PFINT8) buf) - sizeof(struct bhead));
#ifdef BufStats
    ...
    bdh = BDH(((RTP_PFINT8) buf) - sizeof(struct bdhead));
    RTIP_DEBUG_ASSERT(b->bh.prevfree == 0, "brel", EBS_NOVAR, 0, 0);
#ifdef BufStats
    totalloc -= bdh->tsize;
    RTIP_DEBUG_ASSERT(totalloc >= 0, "brel", EBS_NOVAR, 0, 0);
    numdrel++;            /* Number of direct releases */
#endif /* BufStats */
#ifdef FreeWipe
    rtp_memset((RTP_PFINT8) buf, 0x55,
         (MemSize) (bdh->tsize - sizeof(struct bdhead)));
#endif /* FreeWipe */
#if (DEBUG_ASSERT_BGET)
    RTIP_DEBUG_ASSERT(relfcn != 0, "brel: ", EBS_NOVAR, 0, 0);
#endif
...
#if (DEBUG_ASSERT_BGET)
    RTIP_DEBUG_ASSERT(b->bh.bsize < 0, "brel: ", EBS_NOVAR, 0, 0);
#endif
...
#if (DEBUG_ASSERT_BGET)
    RTIP_DEBUG_ASSERT(BH((RTP_PFINT8) b - b->bh.bsize)->prevfree == 0, "brel: ",
        EBS_NOVAR, 0, 0);
#endif
    ...
    totalloc += b->bh.bsize;
    if (totalloc>maxalloc) maxalloc = totalloc;
    RTIP_DEBUG_ASSERT(totalloc >= 0, "brel: totalloc > 0: ", EBS_DINT1, totalloc, 0);
#endif
    ...
    if(!( BH((RTP_PFINT8) b - b->bh.prevfree)->bsize == b->bh.prevfree ))
    {
    	RTIP_DEBUG_ERROR("brel: bsize != prevfree",EBS_NOVAR,0,0);
    }
        
    b = BFH(((RTP_PFINT8) b) - b->bh.prevfree);
    b->bh.bsize -= size;
    } else {
...
#if (DEBUG_ASSERT_BGET)
    RTIP_DEBUG_ASSERT(freelist.ql.blink->ql.flink == &freelist, "brel: prev buffer not alloc", EBS_NOVAR, 0, 0);
#endif
    b->ql.flink = &freelist;
    ...
    bn =  BFH(((RTP_PFINT8) b) + b->bh.bsize);
    if (bn->bh.bsize > 0) {
...
#if (DEBUG_ASSERT_BGET)
    RTIP_DEBUG_ASSERT(BH((RTP_PFINT8) bn + bn->bh.bsize)->prevfree == bn->bh.bsize,
        "brel: next buffer is free", EBS_NOVAR, 0, 0);
    RTIP_DEBUG_ASSERT(bn->ql.blink->ql.flink == bn, "brel: ", EBS_NOVAR, 0, 0);
#endif
...
#ifdef FreeWipe
    rtp_memset(((RTP_PFINT8) b) + sizeof(struct bfhead), 0x55,
        (MemSize) (b->bh.bsize - sizeof(struct bfhead)));
#endif
#if (DEBUG_ASSERT_BGET)
    RTIP_DEBUG_ASSERT(bn->bh.bsize < 0, "brel: dh.bsize < 0", EBS_DINT1,
        bn->bh.bsize, 0);
#endif
...
#if (DEBUG_ASSERT_BGET)
    RTIP_DEBUG_ASSERT(b->bh.prevfree == 0, "brel: ", EBS_NOVAR, 0, 0);
    RTIP_DEBUG_ASSERT(BH((RTP_PFINT8) b + b->bh.bsize)->bsize == ESent, "brel: ", EBS_NOVAR, 0, 0);
    RTIP_DEBUG_ASSERT(BH((RTP_PFINT8) b + b->bh.bsize)->prevfree == b->bh.bsize,
        "brel: ", EBS_NOVAR, 0, 0);
#endif
...
void bpool(void* buf, bufsize len)
{
...
#if (DEBUG_BGET)
    RTIP_DEBUG_ERROR("bpool: commit addr, len", EBS_DINT2, buf, len);
#endif
#ifdef SizeQuant
    len &= ~(SizeQuant - 1);
#endif
    ...
    RTIP_DEBUG_ASSERT(numpblk == numpget - numprel, "bpool: ", EBS_NOVAR, 0, 0);
#endif /* BufStats */
#endif /* BECtl */
    ...
    /* Since the block is initially occupied by a single free  buffer,
     ...
     if(!(len - sizeof(struct bhead) <= -((bufsize) ESent + 1)))
     {
     	RTIP_DEBUG_ERROR("bpool: head >= ESent + 1",EBS_NOVAR,0,0);
     	return;
     }
    ...
    if(!(freelist.ql.blink->ql.flink == &freelist))
    {
    	RTIP_DEBUG_ERROR("flink != &frelist",EBS_NOVAR,0,0);
     	return;
    }
    if(!(freelist.ql.flink->ql.blink == &freelist))
    {
     	RTIP_DEBUG_ERROR("blink !- &freelist",EBS_NOVAR,0,0);
     	return;
    }
    b->ql.flink = &freelist;
    b->ql.blink = freelist.ql.blink;
...
#endif
    bn = BH(((RTP_PFINT8) b) + len);
    bn->prevfree = (bufsize) len;
    /* Definition of ESent assumes two's complement!   */
    if(!((~0) == -1))
    {
       RTIP_DEBUG_ERROR("bpool: -0 != -1 ",EBS_NOVAR,0,0);
       return;
    }
    bn->bsize = ESent;
}
...
void bstats(bufsize *pmaxalloc, bufsize *curalloc, bufsize *totfree, bufsize *maxfree,
            long *nget, long *nrel)
{
    ...
    *maxfree = -1;
    while (b != &freelist) {
        RTIP_DEBUG_ASSERT(b->bh.bsize > 0, "bstats: ", EBS_NOVAR, 0, 0);
        *totfree += b->bh.bsize;
...
void bufdump(void* buf)
{
    ...
    b = BFH(((RTP_PFINT8) buf) - sizeof(struct bhead));
    RTIP_DEBUG_ASSERT(b->bh.bsize != 0, "bufdump: ", EBS_NOVAR, 0, 0);
    if (b->bh.bsize < 0) {
        bdump = (RTP_PFINT8) buf;
        bdlen = (-b->bh.bsize) - sizeof(struct bhead);
    } else {
        bdump = ((RTP_PFINT8) b) + sizeof(struct bfhead);
    ...
    while (bdlen > 0) {
        ...
        bdump += l;
        bdlen -= l;
        while ((bdlen > 16) && (memcmp((RTP_PFINT8) (bdump - 16),
                           (RTP_PFINT8) bdump, 16) == 0)) {
            dupes++;
            bdump += 16;
            bdlen -= 16;
        }
...
void bpoold(void* buf, int dumpalloc, int dumpfree)
{
    ...
    while (b->bh.bsize != ESent) {
        ...
        if (bs < 0) {
            ...
            if (dumpalloc) {
                bufdump((void*) (((RTP_PFINT8) b) + sizeof(struct bhead)));
            }
        } else {
            RTP_PFINT8 lerr = "";

            RTIP_DEBUG_ASSERT(bs > 0, "bpoold: ", EBS_NOVAR, 0, 0);
            if ((b->ql.blink->ql.flink != b) ||
            ...
            V tc_printf("Free block:       size %6ld bytes.%s\n",
                        (long) bs, lerr);
#ifdef FreeWipe
            lerr = ((RTP_PFINT8) b) + sizeof(struct bfhead);
...
int bpoolv(void* buf)
{
    ...
    while (b->bh.bsize != ESent) {
        ...
        if (bs < 0) {
            ...
            RTIP_DEBUG_ASSERT(bs > 0, "bpoolv: ", EBS_NOVAR, 0, 0);
            if (bs <= 0) {
                return 0;
            }
            if ((b->ql.blink->ql.flink != b) ||
                ...
                V tc_printf("Free block: size %6ld bytes.  (Bad free list links)\n",
                         (long) bs);
                RTIP_DEBUG_ASSERT(0, "bpoolv: ", EBS_NOVAR, 0, 0);
                return 0;
            }
#ifdef FreeWipe
            lerr = ((RTP_PFINT8) b) + sizeof(struct bfhead);
...
static RTP_PFINT8 bchain = 0;         /* Our private buffer chain */
static RTP_PFINT8 bp = 0;         /* Our initial buffer pool */
...
static void stats(RTP_PFINT8 when)
{
...
{
    if (bs < sizeof(RTP_PFINT8)) {
        bs = sizeof(RTP_PFINT8);
    }
...
int main(void)
{
    ...
    bp = malloc(ExpIncr);
    RTIP_DEBUG_ASSERT(bp != 0, "main: ", EBS_NOVAR, 0, 0);
    ...
    RTIP_DEBUG_ASSERT(bs <= (((bufsize) 4) * ExpIncr), "main: ", EBS_NOVAR, 0, 0);
    bs = blimit(bs);
    if (rand() & 0x400) {
        cb = (RTP_PFINT8) bgetz(bs);
    } else {
        cb = (RTP_PFINT8) bget(bs);
    }
    if (cb == 0) {
#ifdef EasyOut
        break;
    ...
    *((RTP_PFINT8 *) cb) = (RTP_PFINT8) bchain;
    bchain = cb;
...
#endif
                newb = (RTP_PFINT8) bgetr((void*) fb, bs);
#ifdef BECtl
                protect = 0;
#endif
                if (newb != 0) {
    ...
    while (bchain != 0) {
        RTP_PFINT8 buf = bchain;

        bchain = *((RTP_PFINT8 *) buf);
        brel((void*) buf);
    }
    stats("\nAfter release");
#ifndef BECtl
