                    ...
                    unsigned char *ipAddr, int *port, int *type)
{
    ...
    *connectSock = (RTP_SOCKET)conSocket;
...
/*----------------------------------------------------------------------*
                          rtp_net_connect
 *----------------------------------------------------------------------*/
int rtp_net_connect (RTP_SOCKET sockHandle,
                     ...
                     unsigned char * ipAddr, int port, int type, int flags)
{
    ...
    result = (long) sendto((int) sockHandle, (RTP_PFCCHAR) buffer, (int) size, 0, (PCSOCKADDR)&sin, sinLen);
...
/*----------------------------------------------------------------------*
                           rtp_net_recvfrom
 *----------------------------------------------------------------------*/
long rtp_net_recvfrom (RTP_SOCKET sockHandle,
                       ...
                       unsigned char *ipAddr, int *port, int *type, int flags)
{
    ...
    result = (long) recvfrom ((int) sockHandle, (RTP_PFCHAR) buffer, (int) size, 0, (PSOCKADDR) &remote, (RTP_PFINT) &remoteLen);
 ...
 *----------------------------------------------------------------------*/
int rtp_net_setmembership (RTP_SOCKET sockHandle, unsigned char * ipAddr, int type, unsigned int onBool)
{
    struct sockaddr_in sin;
    struct sockaddr_in localAddr;
    int localLen;
