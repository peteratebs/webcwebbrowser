...
/** @memo   Bind a socket handle to a local port number and optionally
    ...
    error occurred.  A non-fatal error includes a socket that the
    reuse address option has not been set and the socket has
    already been bound to the same IP address indicated in this
    call, indicating that this additional bind call is not needed.
...
/** @memo   Send data over a connection.
    ...
    TCP/IP layer, turn on RTP_DEBUG in rtpdebug.h to display
 ...
 *----------------------------------------------------------------------*/
/** @memo   Set multicast membership option.

    @doc    Add or remove membership to a multicast address.
    ...
    native error value.
 */
int rtp_net_setmembership (
  RTP_HANDLE sockHandle,                /** Socket handle of the socket to manipulate. */
  unsigned char * ipAddr,               /** The address to add or remove mutlicast membership on. */
  int type,                             /** The type of ip address that is passed in:<br>
	<pre>
|		4   Indicates an IPv4 address.
|		6   Indicates an IPv6 address.
	</pre> */
  unsigned int onBool                   /** 1 add membership, 0 remove membership. */
  )
{
    rtp_not_yet_implemented();
    return (-1);
 ...
 *----------------------------------------------------------------------*/
/** @memo   Obtain the ipaddress of a local interface

    @doc    This function is called to obtain the ip address of the local
    interface to which the supplied remote address connects to.

    @return 0 on success -1 otherwise.
 */
int rtp_net_getifaceaddr (
		unsigned char *localAddr,  /* address of the buffer to store local
		                              ip address in */
		unsigned char *remoteAddr, /* ip address of the remote machine which
		                              connects to the local interface */
		int remotePort,            /* port number on the remote machine */
		int remoteType             /* type of ipaddress that is passed */
	)
{
	RTP_SOCKET tempSock;
	int localPort;
	...
	{
		rtp_net_connect(tempSock, remoteAddr, remotePort, remoteType);

		rtp_net_getsockname(tempSock, localAddr, &localPort, &localType);
 ...
 *----------------------------------------------------------------------*/
/** @memo   Allocate stream type (TCP) socket of type IPv6 or IPv4.

    @doc    This function is similar to "rtp_net_socket_stream ( )" with
    the difference being this function supports creation of both IPv6
    and IPv4 sockets depending on the input type.
    Following are excerpt from rtp_net_socket_stream ( ):
    Allocates, if possible, a stream (TCP, connection-based)
    type socket.  The handle then may later be used as an argument to
    the other RTPlatform Socket API functions.  The number of sockets
    available at any one time is dependant on the native TCP/IP
    stack.  When use of the socket handle is completed,
    ...
    native error value.
 */
int rtp_net_socket_stream_dual (
  RTP_HANDLE * sockHandle,              /** Address of a socket handle to store
                                            the allocated socket. */
  int          type                     /** ip version type for the new socket:
                                            type for IPv4-RTP_NET_TYPE_IPV4
                                            type for IPv6-RTP_NET_TYPE_IPV6 */
  )
{
    rtp_not_yet_implemented();
	return (-1);
 ...
 *----------------------------------------------------------------------*/
/** @memo   Allocate datagram type (UDP) socket of type IPv6 or IPv4.

    @doc    This function is similar to "rtp_net_socket_datagram ( )" with
    the difference being this function supports creation of both IPv6
    and IPv4 sockets depending on the input type.
    Following are excerpt from rtp_net_socket_datagram ( ):
    Allocates, if possible, a datagram (UDP, connectionless)
    type socket.  The handle then may later be used as an argument
    to the other RTPlatform Socket API functions.  The number of
    sockets available at any one time is dependant on the native
    TCP/IP stack.  When use of the socket handle is completed,
    ...
    native error value.
 */
int rtp_net_socket_datagram_dual (
  RTP_HANDLE * sockHandle,              /** Address of a socket handle to store
                                            the allocated socket. */
  int          type                     /** ip version type for the new socket:
                                            type for IPv4-RTP_NET_TYPE_IPV4
...
/** @memo   Bind a socket handle to a local port number and optionally
    ...
    @doc   This function supports IPv6 and IPv4 binds. Compared to
    rtp_net_bind ( ) this function takes an additional argument which specifies
    the socket type (TCP OR UDP), such information is needed in certain flavour
    of ipv6 binds.

    This function should be called on a socket before
    calling rtp_net_listen/rtp_net_accept (if the socket is
    stream-type), or rtp_net_recvfrom (if the socket is
    datagram-type).  The behavior of this function is undefined
    if sockHandle is not a valid socket handle.  If the IP
    ...
    For IPv4 datagram-type sockets, in order to receive broadcast and
    multicast packets, the socket handle should be bound to the
    defualt (INADDR_ANY) IP address or to the broadcast or the
    specific multicast address.<br><br>
    ...
    rtpdebug.h to display the native error value.
 */
int rtp_net_bind_dual (
  RTP_HANDLE sockHandle,                /** Socket handle to bind to. */
  int sockType,                         /** Specifies the type of socket.
                                            Supply RTP_NET_STREAM for tcp
                                            RTP_NET_DATAGRAM for udp */
  unsigned char *ipAddr,                /** The optional IP address to be
                                            attached to. */
  int port,                             /** Port (in host byte order) to be
                                            attached to. */
  int type                              /** Type of IP address:<br>
	<pre>
|		4   Indicates an IPv4 address.
 ...
 *----------------------------------------------------------------------*/
/** @memo   Connect a IPv6 or IPv4 socket to a remote IPv6 or IPv4 host.

    @doc    This function supports IPv6 and IPv4 binds. Compared to
    rtp_net_connect ( ) this function takes an additional argument which
    specifies the socket type (TCP OR UDP), such information is needed
    in certain flavour of connects.
    rtp_net_connect establishes a connection between
    a socket and the service provider at the IP address and port
    number provided in the arguments.  If the socket is unbound,
    it will be bound with a unique port number and the local
    connecting IP address for the interface.  Behavior is
