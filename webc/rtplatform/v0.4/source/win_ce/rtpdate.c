...
Returns:
    ...
    layer, turn on RTP_DISPLAY_ERROR in rtpdate.c to display the
    native error value.
----------------------------------------------------------------------*/
...
//time_t timer;
    ...
    //time (&timer);
    winTime = 0;//localtime ((const time_t *)&timer);
    if (winTime == 0)
    {
    ...
    (*date).tzOffset = -5;                 /* signed int; difference in seconds imposed by timezone (from GMT)  */
#endif
    return (0);
}
