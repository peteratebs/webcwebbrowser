...
char * temp = ertfsName;
	...
	fileHandle = po_open ((byte *)ertfsName, _rtp_flag_to_operation(flag), _rtp_mode_to_permission(mode));
...
// Replace Rtfs map functions for now. Rtfs is compiled non unicode, these convert to non unicode
//#if (KS_LITTLE_ENDIAN)
...
char * temp = ertfsName;
	...
	/***** BUG in the smb or may be in this file ****/
	/***** Wild card is not being appended to the root dir path ***/
	/***** Keeping this for now instead of fixing smb to check rtfs with UPNP ****/
	if (ertfsName[0] == '\\' && ertfsName[1] == '\0')
	{
		ertfsName[1] = '*';
		ertfsName[2] = 0;
	}
	else if (ertfsName[1] == ':' && ertfsName[2] == '\\' && ertfsName[3] == '\0')
	{
		ertfsName[3] = '*';
		ertfsName[4] = 0;
	}
