...
int result;
	...
	{
		{
//			FILE* fp = fopen("mutexlog.txt","a+");
//			fprintf(fp, "MUTEX alloc FAILED! - %s (line %d)\n", file, line_num);
//			fclose(fp);
		}
	...
	{
//		FILE* fp = fopen("mutexlog.txt","a+");
//		fprintf(fp, "MUTEX allocated (id: %d) - %s (line %d)\n", nativeMutex, file, line_num);
//		fclose(fp);
	}
...
RTPMutexHeader* pFreedHeader = (RTPMutexHeader*)mtxHandle;
	...
	{
//		FILE* fp = fopen("mutexlog.txt","a+");
//		fprintf(fp, "MUTEX freed (id: %d) - %s (line %d)\n", pFreedHeader->mtx, file, line_num);
//		fclose(fp);
	}
...
int result;
	...
	{
//		FILE* fp = fopen("mutexlog.txt","a+");
//		fprintf(fp, "MUTEX waiting (id: %d) - %s (line %d)\n", pMutexHead->mtx, file, line_num);
//		fclose(fp);
	}
	...
	if (!__rtp_TrackingMutexHeader(pMutexHead))
	{
		...
		/*       exist on the track list.      */
		/* ----------------------------------- */
		result = _rtp_sig_mutex_claim(mtxHandle);
	}
	else
	{
	...
	{
//		FILE* fp = fopen("mutexlog.txt","a+");
//		fprintf(fp, "MUTEX claimed (id: %d) - %s (line %d)\n", pMutexHead->mtx, file, line_num);
//		fclose(fp);
	}
...
RTPMutexHeader* pMutexHead = (RTPMutexHeader*)mtxHandle;
	...
	{
//		FILE* fp = fopen("mutexlog.txt","a+");
//		fprintf(fp, "MUTEX released (id: %d) - %s (line %d)\n", pMutexHead->mtx, file, line_num);
//		fclose(fp);
	}
...
static int __rtp_AddMutexHeader (RTPMutexHeader** pNewMutex, const char* file, long line)
{
	...
	if (__rtp_InitMutexDebug())
	{
		...
		*pNewMutex = gpMutexFreeList;
		...
		rtp_strncpy((*pNewMutex)->file, file, 255);
		(*pNewMutex)->file[255] = 0;
		(*pNewMutex)->line  = line;
