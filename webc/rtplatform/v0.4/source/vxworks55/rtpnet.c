...
{
    int iHostIpAddress = 0;

    if ((iHostIpAddress = hostGetByName((const char *)name)) == ERROR)
    {
#ifdef RTP_DEBUG
...
/*
Is following conversion is right?
*/
    ipAddr[0] = ((unsigned char*) (&iHostIpAddress))[0];
    ipAddr[1] = ((unsigned char*) (&iHostIpAddress))[1];
    ipAddr[2] = ((unsigned char*) (&iHostIpAddress))[2];
    ipAddr[3] = ((unsigned char*) (&iHostIpAddress))[3];
                     ...
                     unsigned char * ipAddr, int port, int type)
{
    ...
    result = sendto((int) sockHandle, (const void *) buffer, (size_t) size, 0, (const struct sockaddr *)&sin, sinLen);
