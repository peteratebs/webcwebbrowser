                     ...
                     int port, int type)
{
    ...
    if (connect((int) sockHandle, (struct sockaddr *) &sin, sinLen) != 0)
    {
        return (-1);
    }
...
unsigned  rtp_net_is_connected    (RTP_SOCKET sockHandle)
{
    ...
    if (getpeername ((int) sockHandle, (struct sockaddr *) &peerAddr, &peerLen) == 0)
	{
		...
		result = selectsocket((int) sockHandle + 1, &tempSet, 0, 0, &selectTime);
		if (result != -1)
		{
...
int result;
	...
	if (msecTimeout >= 0)
    {
		...
		result = selectsocket(1, (fd_set *) 0, (fd_set *) &write_set, (fd_set *) 0, (const struct timeval *) &selectTime);
    }
	else
	{
		result = selectsocket(1, (fd_set *) 0, (fd_set *) &write_set, (fd_set *) 0, (const struct timeval *) NULL);
	}
...
int result;
	...
	if (msecTimeout >= 0)
    {
        selectTime.tv_sec = msecTimeout / 1000;
        selectTime.tv_usec = (msecTimeout % 1000) * 1000;
		result = selectsocket(1, (fd_set *) &read_set, (fd_set *) 0, (fd_set *) 0, (const struct timeval *) &selectTime);
    }
	else
	{
		result = selectsocket(1, (fd_set *) &read_set, (fd_set *) 0, (fd_set *) 0, (const struct timeval *) NULL);
	}
...
long rtp_net_send (RTP_HANDLE sockHandle, const unsigned char * buffer, long size)
{
    ...
    result = send((int) sockHandle, (char *) buffer, (int) size, 0);
                     ...
                     unsigned char * ipAddr, int port, int type)
{
    ...
    result = sendto((int) sockHandle, (char *) buffer, (size_t) size, 0, (struct sockaddr *)&sin, sinLen);
...
{
    if (closesocket((int) sockHandle) == -1)
    {
        return (-1);
    }
                    ...
                    long msecTimeout)
{
	...
	if (msecTimeout >= 0)
    {
        selectTime.tv_sec = msecTimeout / 1000;
        selectTime.tv_usec = (msecTimeout % 1000) * 1000;
		result = selectsocket(highest + 1, (fd_set *) &read_set, (fd_set *) &write_set, (fd_set *) &error_set, (struct timeval *) &selectTime);
    }
	else
	{
		result = selectsocket(highest + 1, (fd_set *) &read_set, (fd_set *) &write_set, (fd_set *) &error_set, (struct timeval *) NULL);
	}
