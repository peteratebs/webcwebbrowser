...
public:
	...
	HTMLEventStatus kbBodyCallbackMethod(wcCtx* Ctx, wcEL element,HTMLEvent* event);
	void KeyBoardRepeatMethod(void);
...
protected:
	...
	void * FocusElement; // Text input element that poped us up.
	void (*closeFn)(void *El, WEBC_CHAR *p); // Close function callback
	WEBC_BOOL ProcessingTextArea;// Working for a text area, allow newlines. 
	WebKey KeyToRepeat;			 // After delay if non zero repeat keyup/keydown. Cleared by mouseup
};
