...
struct flyingwindowcontrol *pcontrol;
	...
	{
		struct flyingwindowcontrol *pcontrol;
		pcontrol = (struct flyingwindowcontrol *)WEBC_MALLOC(sizeof(struct flyingwindowcontrol));
	...
	int left,top,width,height;
		...
		if (pcontrol->iteration >= FLYINGWINDOWITERATIONS)
		{
			...
			wcCtx* KillCtx;
			KillCtx = (wcCtx*)WEBC_MALLOC(sizeof(*Ctx));
			*KillCtx = *Ctx;
	...
	if (CycleTimer)
	{
		...
		wcElTriggerEvent(element, &NewEvent, FLYINGWINDOWPERIOD);
    }
	return (HTML_EVENT_STATUS_CONTINUE);
}
static void StartFlyingWindowDemo( int top, int left,WEBC_UINT32 WindowFlags)
...
static void CycleTest10(void)
{
	...
	if (Test10CycleCount < FOCUSTESTCOUNT)
		wcElTriggerEvent(Test10Elements[0][0], &Event, 500);
...
int top,left,width,height;
	...
	if (!wcElGetPosStyle(El, &top,&left,&height,&width))
