...
unsigned short giGb2312toUnicode_page21 [] =
{
  /* 0x21 */
...
unsigned short giGb2312toUnicode_page30[] = {
  /* 0x30 */
...
static const GB2312Summary16 gb2312_uni2indx_page00[70] = {
  /* 0x0000 */
...
static const GB2312Summary16 gb2312_uni2indx_page20[101] = {
  /* 0x2000 */
...
static const GB2312Summary16 gb2312_uni2indx_page30[35] = {
  /* 0x3000 */
...
static const GB2312Summary16 gb2312_uni2indx_page4e[1263] = {
  /* 0x4e00 */
...
static const GB2312Summary16 gb2312_uni2indx_page9e[27] = {
  /* 0x9e00 */
...
static const GB2312Summary16 gb2312_uni2indx_pageff[15] = {
  /* 0xff00 */
...
{
	unsigned char c1 = gb2312[0] & 0x7f; // GB2312 is wrapped in EUC-CN

	if ((gb2312[0] & 0x80) && ( (c1 >= 0x21 && c1 <= 0x29) || (c1 >= 0x30 && c1 <= 0x77))) 
	{
		unsigned char c2 = gb2312[1] & 0x7f; // unwrap EUC-CN
		if (c2 >= 0x21 && c2 < 0x7f) 
		{
			...
			{
				if (i < 831)
				wc = giGb2312toUnicode_page21[i];
			} 
			else 
			{
				if (i < 8178)
				wc = giGb2312toUnicode_page30[i-1410];
			}
			if (wc != 0xfffd) 
			{
				*uc = (unsigned short) wc;
				return (2);
			}
		}
	}

	uc[0] = (unsigned short) gb2312[0];
	return (1);
}

int webc_unicode_to_gb2312(unsigned char *gb2312, unsigned short uc)
{
    if (uc < 0x80) /* Ascii - not sure check if upper or lower */
    {
        gb2312[0] = (WEBC_UINT8) uc;
        return(1);
    }

    const GB2312Summary16 *summary = 0;
    if (uc >= 0x0000 && uc < 0x0460)
      summary = &gb2312_uni2indx_page00[(uc>>4)];
    else if (uc >= 0x2000 && uc < 0x2650)
      summary = &gb2312_uni2indx_page20[(uc>>4)-0x200];
    else if (uc >= 0x3000 && uc < 0x3230)
      summary = &gb2312_uni2indx_page30[(uc>>4)-0x300];
    else if (uc >= 0x4e00 && uc < 0x9cf0)
      summary = &gb2312_uni2indx_page4e[(uc>>4)-0x4e0];
    else if (uc >= 0x9e00 && uc < 0x9fb0)
      summary = &gb2312_uni2indx_page9e[(uc>>4)-0x9e0];
    else if (uc >= 0xff00 && uc < 0xfff0)
      summary = &gb2312_uni2indx_pageff[(uc>>4)-0xff0];
    if (summary) {
      unsigned short used = summary->used;
      unsigned int i = uc & 0x0f;
      if (used & ((unsigned short) 1 << i)) {
        ...
        gb2312[0] = (c >> 8) | 0x80; // wrap g2312 in EUC-CN
		gb2312[1] = (c & 0xff) | 0x80;
        return (2);
      }
    }
	gb2312[0] = '?';
