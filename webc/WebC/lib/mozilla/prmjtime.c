...
#ifdef __WEBC__
static JSInt64 prev_utc_date = 0;
static long base_ms = 0;
#endif
...
PRMJ_Now(void)
{
    ...
    long ms  = rtp_get_system_msec();

    JSLL_UI2L(ms2us, PRMJ_USEC_PER_MSEC);
    JSLL_UI2L(s2us, PRMJ_USEC_PER_SEC);
    JSLL_UI2L(s, rtp_get_utc_date());
	// A hack to eliminate problems when adding milisecond clock and utc clock
	//   They are not synchronized so the second clock rolls over to the next highest but the millisecond
	//   clock doesn't cross the 0 (100) threshold. This causes big jumps in jquery animation
	//   Fix it by saving the milisecond offset when we read a new second value and subtract that
	//   value from the mili clock before we add it in.
