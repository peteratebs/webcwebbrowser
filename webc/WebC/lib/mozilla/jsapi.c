...
#ifdef __WEBC__
    rt = (JSRuntime *) WEBC_JSMALLOC(sizeof(JSRuntime));
#else
...
JS_PUBLIC_API(void *)
	_JSDB_malloc(JSContext *cx, size_t nbytes,char *file, long line)
#else
JS_PUBLIC_API(void *)
	_JSDB_malloc(JSContext *cx, size_t nbytes)
#endif
{
    void *p;
...
#ifdef __WEBC__
#ifdef RTP_TRACK_LOCAL_MEMORY
	p = _rtp_debug_malloc(nbytes, _rtp_malloc, file, line,"JSDB_malloc",0);
#else
    p = WEBC_JSMALLOC(nbytes);
#endif
#else
	p = malloc(nbytes);
#endif
