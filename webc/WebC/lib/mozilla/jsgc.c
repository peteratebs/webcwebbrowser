...
#ifdef __WEBC__
		array = (void **) WEBC_JSREALLOC(table->array,
...
#ifdef __WEBC__
        array = (void **) WEBC_JSREALLOC(array, capacity * sizeof array[0]);
#else
...
# endif
     ...
     */
#ifdef __WEBC__
	p = WEBC_JSMALLOC((js_gcArenasPerChunk + 1) << GC_ARENA_SHIFT);
#else
...
# endif
 ...
 #ifdef __WEBC__
	WEBC_JSFREE((void *) (chunk - *GetMallocedChunkGapPtr(chunk)));
#else
