               ...
               JSString *sep, jsval *rval)
{
    ...
    if (op == TO_SOURCE) {
        ...
        if (!chars) {
            nchars = 0;
#ifdef __WEBC__
            chars = (jschar *) WEBC_JSMALLOC(growth);
#else
...
#ifdef __WEBC__
            chars = (jschar *)WEBC_JSREALLOC((ochars = chars), growth);
#else
...
#ifdef __WEBC__
	// if we're passed a collection, then we really don't know how to convert it to an array
	// (although we could probably make a good try at it). Therefore, if we get a non-empty
	// collection, then throw a JavaScript exception (mainly for the benifit of jQuery).

	if(length && !JS_IsArrayObject(cx, obj) && JSVAL_IS_OBJECT(*vp))
	{
		jsval e = INT_TO_JSVAL(0);
