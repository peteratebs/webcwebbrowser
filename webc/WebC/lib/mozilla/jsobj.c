...
obj_toSource(JSContext *cx, uintN argc, jsval *vp)
{
    ...
    if (!chars) {
...
#ifdef __WEBC__
        chars = (jschar *) WEBC_JSMALLOC(((outermost ? 4 : 2) + 1) * sizeof(jschar));
#else
        ...
        chars = (jschar *)
#ifdef __WEBC__
			WEBC_REALLOC((ochars = chars), (nchars + 2 + 1) * sizeof(jschar));
#else
...
#undef SAFE_ADD
            ...
            chars = (jschar *)
#ifdef __WEBC__
			WEBC_REALLOC((ochars = chars), curlen * sizeof(jschar));
#else
...
#ifdef __WEBC__
        slots = (jsval *)WEBC_REALLOC(old, nwords * sizeof(jsval));
#else
