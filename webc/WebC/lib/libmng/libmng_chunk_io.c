                                      ...
                                      mng_uint32 *iRealsize)
{
  ...
  if (iInsize)                         /* anything to do ? */
  {
    *iOutsize = iInsize * 3;           /* estimate uncompressed size */
    ...
    MNG_ALLOC (pData, temp, *iOutsize)
    *pOutbuf = (mng_uint8p)temp;
...
READ_CHUNK (mng_read_iccp)
{
  ...
  if ((iRetcode) && (!strncmp ((char *)pRawdata, "Photoshop ICC profile", 21)))
  {
    if (iRawlen == 2615)               /* is it the sRGB profile ? */
    {
      ...
      iRetcode = mng_read_srgb (pData, &chunk_srgb, 1, (mng_uint8p)"0", ppChunk);
...
READ_CHUNK (mng_read_text)
{
  ...
  if (pData->fProcesstext)             /* inform the application ? */
  {
    ...
    MNG_ALLOC (pData, temp, iKeywordlen + 1)
    zKeyword = (char*)temp;
    MNG_COPY  (zKeyword, pRawdata, iKeywordlen)

    MNG_ALLOCX (pData, temp, iTextlen + 1)
    zText = (char*)temp;
...
READ_CHUNK (mng_read_ztxt)
{
  ...
  {                                    /* decompress the text */
    ...
    MNG_ALLOCX (pData, temp, iKeywordlen+1)
	zKeyword = (char*)temp;
...
READ_CHUNK (mng_read_itxt)
{
  ...
  if (pData->fProcesstext)             /* inform the application ? */
  {
    ...
    else
    {
      ...
      MNG_ALLOCX (pData, temp, iBufsize);
      pBuf = (mng_uint8p)temp;
      MNG_COPY   (pBuf, pNull3+1, iTextlen);
    }

    MNG_ALLOCX (pData, temp,     iKeywordlen     + 1)
    zKeyword = (char*)temp;
    MNG_ALLOCX (pData, temp,    iLanguagelen    + 1)
    zLanguage = (char*)temp;
    MNG_ALLOCX (pData, temp, iTranslationlen + 1)
	zTranslation = (char*)temp;
                                       /* on error bail out */
...
READ_CHUNK (mng_read_disc)
{
  ...
  {
    void *temp;
    MNG_ALLOC (pData, temp, iRawlen)
    pIds = (mng_uint16p)temp;
...
READ_CHUNK (mng_read_seek)
{
  ...
  if (pData->fProcessseek)             /* inform the app ? */
  {
    ...
    MNG_ALLOC (pData, temp, iRawlen + 1)
    zName = (char*)temp;
