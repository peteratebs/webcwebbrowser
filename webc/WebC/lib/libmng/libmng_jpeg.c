...
#ifdef MNG_INCLUDE_JNG_READ
#ifdef MNG_DEFINE_JPEG_STDCALL
mng_bool MNG_DECL mng_fill_input_buffer (j_decompress_ptr cinfo)
#else
mng_bool mng_fill_input_buffer (j_decompress_ptr cinfo)
#endif
{
...
mng_retcode mngjpeg_initialize (mng_datap pData)
{
  ...
  if (pData->pJPEGderr   == MNG_NULL)
    MNG_ALLOC (pData, temp,   sizeof (mngjpeg_error ))
    pData->pJPEGderr = (mngjpeg_error*)temp;
  if (pData->pJPEGdsrc   == MNG_NULL)
    MNG_ALLOC (pData, temp,   sizeof (mngjpeg_source))
    pData->pJPEGdsrc = (mngjpeg_sourcep)temp;
  if (pData->pJPEGdinfo  == MNG_NULL)
    MNG_ALLOC (pData, temp,  sizeof (mngjpeg_decomp))
    pData->pJPEGdinfo = (mngjpeg_decompp)temp;
                                       /* enable reverse addressing */
  pData->pJPEGdinfo->client_data  = pData;

  if (pData->pJPEGderr2  == MNG_NULL)
    MNG_ALLOC (pData, temp,  sizeof (mngjpeg_error ))
    pData->pJPEGderr2 = (mngjpeg_error*)temp;
  if (pData->pJPEGdsrc2  == MNG_NULL)
    MNG_ALLOC (pData, temp,  sizeof (mngjpeg_source))
    pData->pJPEGdsrc2 = (mngjpeg_sourcep)temp;
  if (pData->pJPEGdinfo2 == MNG_NULL)
    MNG_ALLOC (pData, temp, sizeof (mngjpeg_decomp))
    pData->pJPEGdinfo2 = (mngjpeg_decompp)temp;
                                       /* enable reverse addressing */
  ...
  if (pData->pJPEGbuf   == MNG_NULL)   /* initialize temporary buffers */
  {
    ...
    MNG_ALLOC (pData, temp, pData->iJPEGbufmax)
    pData->pJPEGbuf = (mng_uint8p)temp;
  }

  if (pData->pJPEGbuf2  == MNG_NULL) 
  {
    pData->iJPEGbufmax2    = MNG_JPEG_MAXBUF;
    MNG_ALLOC (pData, temp, pData->iJPEGbufmax2)
    pData->pJPEGbuf2 = (mng_uint8p)temp;
  }
  ...
  {                                    /* need to shift anything ? */
    ...
    if (!pData->bJPEGhasheader2)       /* haven't got the header yet ? */
    {
      ...
      {                                /* indicate the header's oke */
        ...
        MNG_ALLOC (pData, temp, pData->iJPEGrowlen2)
        pData->pJPEGrow2 = (mng_uint8p)temp;
