...
{
#if (MAX_VIRTUAL_DISPLAYS > 0)
	if (virtualDisplayBackGroundDraw(ctx))	{	return(virtualDisplayGetBaseAddress(ctx));	}
#endif
	...
	)
{
	...
	rectangle->left	= 0;
	rectangle->top	 = 0;
	rectangle->right  = GET_FBGRAPHICS_CTX(ctx)->frameBufferItf->GetWidth()-1; //  -1;
	rectangle->bottom = GET_FBGRAPHICS_CTX(ctx)->frameBufferItf->GetHeight()-1; // -1;
	return;
#endif
...
static int open = 0;
	...
	if (config->graphics)
	{
		rtp_printf("Not reinitializing the frame buffer if graphics already configured !!!!!!!!!!!!\n");
	...
	{
#ifdef WIN32
	fbItf = FrameBufferDriver::Only();
#else
#ifdef __TIARM__
	fbItf = LocostoFrameBuffer::Only();
#else
	fbItf = EmbeddedBitMapFrameBuffer::Only();
#endif
#endif
	}
...
#if (MAX_VIRTUAL_DISPLAYS > 0)
			if (ctx->browserHandle)	{virtualMapBrowserHandleToCurrentFrame((HTMLGraphicsContext **)ctx);}
#endif
#if (INCLUDE_WINDOW_MANAGER)
