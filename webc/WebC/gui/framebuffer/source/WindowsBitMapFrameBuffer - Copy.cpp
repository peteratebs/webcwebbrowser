...
//	short keyValue;
	...
	switch(message){
		case WM_RBUTTONDOWN: //change this case to be mouse drivers right button down
			...
			send_to_webc = mouse_button_event(RBUTTON_DOWN,x,y,&e);
//			rtp_debug_print_mem_usage (0,0,"Clicked");
		...
		case WM_KEYDOWN://change this case to be the keyboard drivers key down
			...
			*/
			send_to_webc = keyboard_event(KEY_DOWN, pcKbToWebKey((short) (HIWORD(lParam) & 0xff)), &e);
		...
		case WM_KEYUP: //change this case to be the keyboard drivers key up
			...
			*/
			send_to_webc = keyboard_event(KEY_UP, pcKbToWebKey((short) (HIWORD(lParam) & 0xff)), &e);
			break;
// ===
		case WM_TIMER:
		{
...
#endif
			call_TerminalTimout(0);	/* See if we have any remote screen data to display */

			BeginPaint(hWnd, &ps);
			EndPaint(hWnd, &ps);
			send_to_webc = webc_timer_event(&e);
