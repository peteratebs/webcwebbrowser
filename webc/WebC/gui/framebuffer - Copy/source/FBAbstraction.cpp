...
#if (MAX_VIRTUAL_DISPLAYS > 0)
	inBackGroundDraw = virtualDisplayBackGroundDraw(ctx);
#endif
...
#if WIN32
int WindowsFB_ProcessInputQueue(void *fn,WEBC_BOOL  block,WEBC_INT32 *exitCode);
#endif
...
{
#if WIN32
	return WindowsFB_ProcessInputQueue(fn,block,exitCode);
#else
#if (WEBC_SUPPORT_DEBUG_PROFILE)
...
{
#if (MAX_VIRTUAL_DISPLAYS > 0)
	if (VirtualDisplayInit(windowHandle) < 0)	return(-1);
#endif
