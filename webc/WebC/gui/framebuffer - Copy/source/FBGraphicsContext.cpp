...
FBGraphicsContext::FBGraphicsContext(int width, int height)
{
	...
	currentGBuffer = 0;
	clipBounds.bottom	= (DISPLAY_INT) (WEBC_SCREEN_HEIGHT -1); //239;
	clipBounds.top		= (DISPLAY_INT)0;
	clipBounds.left		= (DISPLAY_INT)0;
	clipBounds.right	= (DISPLAY_INT)(WEBC_SCREEN_WIDTH -1); //319;
...
FBGraphicsContext::FBGraphicsContext(HTMLBrowserConfig *config, int width, int height)
{
	...
	clipBounds.bottom	= (DISPLAY_INT) (height-1); //239;
	clipBounds.top		= (DISPLAY_INT)0;
	clipBounds.left		= (DISPLAY_INT)0;
	clipBounds.right	= (DISPLAY_INT)(width-1); //319;
