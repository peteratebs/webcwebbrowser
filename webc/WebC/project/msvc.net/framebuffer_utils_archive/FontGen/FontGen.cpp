...
public:
        ...
        outputPrefix(__T("o"), __T("Output path prefix."))
    {
        ...
        AddFlag(familyName);
        familyName.SetDefaultValue(__T("Arial"));

        AddFlag(genericName);
        genericName.SetDefaultValue(__T("monospace"));
...
FB_Font* createFont (
	...
	_stprintf((wchar_t *)baseFileName,_T("%s%s%dB%04xE%04x"), familyName, style, (int) pointSize, (int) startChar, (int) endChar);
	...
	wcscpy((wchar_t*)new_font->font_family, (const wchar_t*)familyName);
...
#define DEBUG_IMAGE_OUTPUT 1
	...
	_ftprintf(source_file,_T("unsigned char %s_file[] = { \n\n"), baseFileName);
	...
	_ftprintf(h_file,_T("extern unsigned char %s_file[];\n\n"), baseFileName);
