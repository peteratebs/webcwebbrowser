...
#else
  // Init delay timer
  PCONP_bit.PCTIM3 = 1; // Enable TIM3 clocks
  T3TCR = 2;            // stop and reset timer 3
  T3CTCR_bit.CTM = 0;   // Timer Mode: every rising PCLK edge
  T3MCR_bit.MR0S = 1;   // stop timer if MR0 matches the TC
  T3MCR_bit.MR0R = 1;   // enable timer reset if MR0 matches the TC
