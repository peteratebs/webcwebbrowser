...
/************************************************************************
 * RTP_FD_SETSIZE - number of sockets rtp_net_select                   *
 ************************************************************************/
/* SPRZ - where does this belong? porting issue */
#ifndef RTP_FD_SETSIZE
#define RTP_FD_SETSIZE 255
#endif
...
//#include "rtipapi.h"  /* needed for errnos but rtipapi.h includes socket.h */
...
#define RTP_NET_EPFNOSUPPORT            EAFNOSUPPORT    
...
long     rtp_net_recv_flags      (RTP_SOCKET sockHandle, unsigned char *buffer, long size, int flags);
...
long     rtp_net_recvfrom        (RTP_SOCKET sockHandle, unsigned char *buffer, long size,
                                  unsigned char *ipAddr, int *port, int *type);
long     rtp_net_recvfrom_flags  (RTP_SOCKET sockHandle, unsigned char *buffer, long size,
                                  unsigned char *ipAddr, int *port, int *type, int flags);
...
/* Calls to support IPv4, IPv6, dual stack for HTTP Client */
